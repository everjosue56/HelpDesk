...
import { FiHome, FiMapPin, FiShield, FiUsers, FiFileText, FiBriefcase } from 'react-icons/fi';
  ...
  const menuGroups = [
    {
      title: "PRINCIPAL",
      items: [
        { name: "Inicio", path: "/dashboard", icon: FiHome },
      ]
    },
    {
      title: "ADMINISTRATIVO",
      items: [
        { name: "Organizacion", path: "/dashboard/organizations", icon: Building2  },
        { name: "Agencia", path: "/dashboard/agencies", icon: FiBriefcase }, 
        { name: "Area", path: "/dashboard/areas", icon: FiMapPin },
        { name: "Roles y Permisos", path: "/dashboard/roles", icon: FiShield },
        { name: "Usuarios", path: "/dashboard/users", icon: FiUsers },
        { name: "Logs de Auditoria", path: "/dashboard/logs", icon: FiFileText },
      ]
    }
  ];
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-6 clean-scrollbar">
        {menuGroups.map((group, gIdx) => (
          <div key={gIdx} className="space-y-1.5">
            <span className="text-[11px] font-bold text-neutral-400 tracking-widest px-3 block">
              {group.title}
            </span>
