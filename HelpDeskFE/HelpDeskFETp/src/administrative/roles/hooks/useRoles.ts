      ...
      const formattedData: RoleItem[] = Array.isArray(rawData)
        ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
          rawData.map((item: any) => {
            ...
            // Valores por defecto
            let description =
              "Interfaz operativa para el usuario solicitante. Permite la autogestión en la apertura de reportes de incidentes, seguimiento en tiempo real del estado de sus requerimientos y la actualización segura de sus credenciales de acceso..";
            let permissionLevel = "Usuario Final";

            // 1. Caso Administrador
            if (
              lowerName.includes("administrador") ||
              lowerName.includes("admin")
            ) {
              description =
                "Supervisión global de la plataforma. Posee facultades plenas para la auditoría del sistema, aprovisionamiento de infraestructura corporativa, configuración de seguridad y gobernanza sobre el ciclo de vida de agencias, áreas y usuarios.";
              permissionLevel = "Acceso Total";
            }
            // 2. Caso Técnico / TI
            else if (
              lowerName.includes("tecnico") ||
              lowerName === "ti" ||
              lowerName.includes("soporte") ||
              lowerName.includes("it")
            ) {
              description =
                "Gestión operativa y técnica del Service Desk. Encargado del control y trazabilidad de inventarios informáticos, así como del ciclo integral de atención, diagnóstico, escalamiento y resolución de tickets e incidentes asignados.";
              permissionLevel = "Soporte Técnico";
            }
            // 3. Caso Cliente / Usuario Final explicito
            else if (
              lowerName.includes("cliente") ||
              lowerName.includes("usuario")
            ) {
              description =
                "Interfaz operativa para el usuario solicitante. Permite la autogestión en la apertura de reportes de incidentes, seguimiento en tiempo real del estado de sus requerimientos y la actualización segura de sus credenciales de acceso.";
