...
export const useOrganizations = (
  ...
  const organizationService = useMemo(
    () => getOrganizations(AXIOS_INSTANCE),
    [],
  );
  ...
  const getOrganizationById = useCallback(
    async (id: number): Promise<OrganizationItem | null> => {
      try {
        setIsFetching(true);
        const response = await organizationService.getApiOrganizationsId(id);

        const backendResponse =
          response.data as unknown as OrganizationDtoResponseDto;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        console.error(
          `Error al obtener detalle de organización con id ${id}:`,
          error,
        );
        setOrganization(null);
        return null;
