...
interface TypeMaintenanceDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const TypeMaintenanceDeleteModal: React.FC<TypeMaintenanceDeleteModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  typeMaintenance,
  ...
  const handleConfirm = async () => {
    try {
      ...
      await onConfirm(typeMaintenance.id);
      onClose();
    } catch (error) {
      ...
      <AlertDialogContent className="sm:max-w-125 w-[90%] rounded-3xl p-8 bg-white border-none shadow-2xl flex flex-col gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
        ...
        <div className="text-center flex flex-col items-center gap-4 py-2 px-1 w-full">
          ...
          <p className="text-xs sm:text-sm font-bold text-neutral-500 leading-relaxed max-w-95 mx-auto">
            Al eliminar la configuración "{typeMaintenance.name}", esta clasificación y su tiempo estimado de "{typeMaintenance.estimatedTime}" ya no podrán ser seleccionados en las órdenes de trabajo futuras ni en los reportes operativos del inventario.
          </p>
        </div>
        ...
        <div className="flex flex-row items-center justify-center gap-3 w-full">
          ...
          >
            ...
            <span>
              {isSubmitting ? "Eliminando..." : "Sí, Eliminar Tipo"}
            </span>
          </Button>
