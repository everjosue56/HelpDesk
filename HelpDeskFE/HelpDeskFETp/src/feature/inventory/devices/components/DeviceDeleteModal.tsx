...
export const DeviceDeleteModal: React.FC<DeviceDeleteModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  device,
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.error("Error al procesar la baja del dispositivo de hardware:", error);
    } finally {
      ...
      <AlertDialogContent className="sm:max-w-125 w-[90%] rounded-3xl p-8 bg-white border-none shadow-2xl flex flex-col gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
        ...
        <div className="text-center flex flex-col items-center gap-4 py-2 px-1 w-full">
          <p className="text-[14px] font-black text-red-500 tracking-wide uppercase max-w-sm leading-snug">
            ¿Estás seguro que quieres dar de baja este dispositivo?
          </p>
          <p className="text-xs sm:text-sm font-bold text-neutral-500 leading-relaxed max-w-95 mx-auto">
            Al desactivar el equipo <span className="text-slate-800 font-extrabold">"{device.brandName}"</span> con código patrimonial <span className="text-slate-800 font-mono font-bold bg-slate-50 px-1.5 py-0.5 border border-slate-100 rounded">{device.code}</span>, el activo pasará a un estado inactivo. Esto impedirá que sea seleccionado para nuevos tickets de soporte técnico o asignaciones operativas directas hasta que sea reactivado.
          </p>
        </div>
        ...
        <div className="flex flex-row items-center justify-center gap-3 w-full">
          ...
          >
            ...
            <span>
              {isSubmitting ? "Desactivando..." : "Sí, Dar de Baja Equipo"}
            </span>
          </Button>
