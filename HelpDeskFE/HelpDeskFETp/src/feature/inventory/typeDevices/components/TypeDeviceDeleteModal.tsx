...
interface TypeDeviceDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const TypeDeviceDeleteModal: React.FC<TypeDeviceDeleteModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  typedevice,
  ...
  const handleConfirm = async () => {
    try {
      ...
      await onConfirm(typedevice.id);
      onClose();
    } catch (error) {
      console.error("Error al desactivar el tipo de dispositivo:", error);
    } finally {
      ...
      <AlertDialogContent className="sm:max-w-125 w-[90%] rounded-3xl p-8 bg-white border-none shadow-2xl flex flex-col gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
        ...
        <div className="text-center flex flex-col items-center gap-4 py-2 px-1 w-full">
          <p className="text-[14px] font-black text-red-500 tracking-wide uppercase max-w-sm leading-snug">
            ¿Estás seguro que quieres desactivar este tipo de dispositivo?
          </p>
          <p className="text-xs sm:text-sm font-bold text-neutral-500 leading-relaxed max-w-95 mx-auto">
            Al desactivar la categoría "{typedevice.name}", los dispositivos de hardware físicos vinculados a esta clasificación perderán su integridad operativa inmediata al momento de registrar nuevos mantenimientos o auditorías en el inventario.
          </p>
        </div>
        ...
        <div className="flex flex-row items-center justify-center gap-3 w-full">
          ...
          >
            ...
            <span>
              {isSubmitting ? "Desactivando..." : "Sí, Desactivar Tipo"}
            </span>
          </Button>
