...
import type { AlertTypeItem } from '../hooks/useAlertTypes';

interface AlertTypeDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (id: number) => Promise<void>;
  alertType: AlertTypeItem | null; 
}

export const AlertTypeDeleteModal: React.FC<AlertTypeDeleteModalProps> = ({ isOpen, onClose, onConfirm, alertType }) => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      await onConfirm(alertType.id);
      onClose();
    } catch (error) {
      console.error("Error al desactivar el tipo de alerta:", error);
    } finally {
    ...
    <AlertDialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <AlertDialogContent className="sm:max-w-125 w-[90%] rounded-3xl p-8 bg-white border-none shadow-2xl flex flex-col gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
        ...
        <div className="text-center flex flex-col items-center gap-4 py-2 px-1 w-full">
          <p className="text-[14px] font-black text-red-500 tracking-wide uppercase max-w-sm leading-snug">¿Estás seguro que quieres desactivar este tipo de alerta?</p>
          <p className="text-xs sm:text-sm font-bold text-neutral-500 leading-relaxed max-w-95 mx-auto">
            Al desactivar la clasificación "{alertType.name}", los nuevas notificaciones levantados no podrán categorizarse bajo este concepto, alterando las métricas globales del HelpDesk.
          </p>
        </div>
        ...
        <div className="flex flex-row items-center justify-center gap-3 w-full">
          ...
          <Button type="button" onClick={handleConfirm} disabled={isSubmitting} className="flex-[1.5] sm:flex-none sm:w-56 bg-[#ef4444] hover:bg-[#dc2626] text-white flex items-center justify-center gap-1.5 font-bold h-10 rounded-xl text-xs transition-colors shadow-sm cursor-pointer uppercase tracking-wider whitespace-nowrap border-none">
            ...
            <span>{isSubmitting ? "Desactivando..." : "Sí, Desactivar Alerta"}</span>
          </Button>
        </div>
      </AlertDialogContent>
    </AlertDialog>
