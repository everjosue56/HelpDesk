...
import type { NotificationItem } from '../hooks/useNotificationsTable';

interface NotificationsDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const NotificationDeleteModal: React.FC<NotificationsDeleteModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  notification,
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.error("Error al desactivar la notificacion:", error);
    } finally {
      ...
      <AlertDialogContent className="sm:max-w-125 w-[90%] rounded-3xl p-8 bg-white border-none shadow-2xl flex flex-col gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
        ...
        <div className="text-center flex flex-col items-center gap-4 py-2 px-1 w-full">
          <p className="text-[14px] font-black text-red-500 tracking-wide uppercase max-w-sm leading-snug">
            ¿Estas seguro que quieres eliminar esta notificacion?
          </p>
          <p className="text-xs sm:text-sm font-bold text-neutral-500 leading-relaxed max-w-95 mx-auto">
            Al Desactivar la notificacion "{notification.alertTypeName}", los tickets y usuarios asociados perderán la vinculación directa de forma inmediata en el sistema.
          </p>
        </div>
        ...
        <div className="flex flex-row items-center justify-center gap-3 w-full">
          ...
          >
            ...
            <span>
              {isSubmitting ? "Desactivando..." : "Si, Eliminar"}
            </span>
          </Button>
