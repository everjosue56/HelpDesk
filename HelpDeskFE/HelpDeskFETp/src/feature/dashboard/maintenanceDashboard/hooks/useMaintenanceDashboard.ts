...
export const normalizeMaintenanceDashboardPayload = (
  ...
  const record = candidate as Partial<MaintenanceDashboardData> &
  ...
  return {
    ...
    porEstado: Array.isArray(record.porEstado)
      ? (record.porEstado as MaintenanceStatusDto[])
      : [],
    porFrecuencia: Array.isArray(record.porFrecuencia)
      ? (record.porFrecuencia as MaintenanceFrequencyChartDto[])
      : [],
    porArea: Array.isArray(record.porArea)
      ? (record.porArea as MaintenanceAreaChartDto[])
      : [],
    historialMensual: Array.isArray(record.historialMensual)
      ? (record.historialMensual as MaintenanceMonthlyHistoryDto[])
      : [],
  };
};
...
export const useMaintenanceDashboard = (initialYear: number = 2026) => {
  ...
  const fetchDashboardData = useCallback(
    async (
      ...
      try {
        ...
        if (isMounted) {
          const normalizedData = normalizeMaintenanceDashboardPayload(
            response.data,
          );
        ...
        console.error(
          "Error al obtener las métricas del dashboard de mantenimiento:",
          error,
        );
  ...
  const downloadExcel = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `Reporte_Mantenimientos_${year}_${nombreMes}.xlsx`,
      );
