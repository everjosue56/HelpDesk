...
export interface TypeErrorFormValues {
    name: string;
}

interface TypeErrorFormProps {
    initialData?: (TypeErrorFormValues & { id?: number }) | null; 
    onSubmit: (data: TypeErrorFormValues) => Promise<void>;
    onCancel: () => void;
    isSubmitting?: boolean;
}

export const TypeErrorForm: React.FC<TypeErrorFormProps> = ({
    initialData,
    onSubmit,
    onCancel,
    isSubmitting = false,
    ...
    const form = useForm<TypeErrorFormValues>({
        mode: 'onBlur',
        defaultValues: {
            name: '',
        },
            ...
            <div className="flex items-center gap-4 border-b border-gray-100 pb-5">
                ...
                <div>
                    <h2 className="text-xl font-bold text-slate-800">
                        {isEditMode ? 'Editar Tipo de Error' : 'Nuevo Tipo de Error'}
                    </h2>
                    <p className="text-sm text-gray-400 mt-0.5">
                        {isEditMode
                            ? 'Modifica el campo necesario para actualizar la clasificación del reporte.'
                            : 'Completa el formulario para registrar una nueva tipificación de error en el soporte.'}
                    </p>
                </div>
            </div>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-5">
                        ...
                        {/* Campo: Nombre del Tipo de Error */}
                        <FormField
                            control={form.control}
                            name="name"
                            rules={{
                                required: 'El nombre del tipo de error es obligatorio.',
                                maxLength: { value: 100, message: 'El nombre no puede exceder los 100 caracteres.' }
                            }}
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-sm font-bold text-slate-700">Nombre del Tipo de Error</FormLabel>
                                    <FormControl>
                                        <Input 
                                            placeholder="Ej. Errores de software, Fallo de Red" 
                                            {...field} 
