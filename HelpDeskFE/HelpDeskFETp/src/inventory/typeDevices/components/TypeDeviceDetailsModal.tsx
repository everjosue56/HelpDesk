...
import type { TypeDeviceItem } from '../hooks/useTypeDevices';

interface TypeDeviceDetailModalProps {
    isOpen: boolean;
    onClose: () => void;
    typedevice: TypeDeviceItem | null;
}

export const TypeDeviceDetailModal: React.FC<TypeDeviceDetailModalProps> = ({
    isOpen,
    onClose,
    typedevice,
}) => {
    if (!typedevice) return null;
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="sm:max-w-md w-[90%] rounded-2xl p-6 bg-white border border-gray-100 shadow-2xl gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
                ...
                <DialogHeader className="text-left flex flex-row items-center gap-4 pb-4 border-b border-gray-100">
                    ...
                    <div className="space-y-0.5">
                        ...
                        <DialogDescription className="text-xs font-medium text-gray-400">
                            Ficha técnica e información del registro de hardware
                        </DialogDescription>
                    </div>
                </DialogHeader>
                ...
                <div className="py-5 space-y-4 text-left">
                    ...
                    {/* Fila: Nombre del Tipo de Dispositivo */}
                    <div className="flex gap-3 items-start">
                        <FiMonitor className="h-4 w-4 mt-0.5 text-gray-400 shrink-0" />
                        <div className="space-y-0.5">
                            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Tipo de Dispositivo</span>
                            <span className="text-sm font-semibold text-slate-700 block select-text">{typedevice.name}</span>
                        </div>
                    </div>

                    {/* Fila: Descripción del Registro */}
                    <div className="flex gap-3 items-start">
                        <FileText className="h-4 w-4 mt-0.5 text-gray-400 shrink-0" />
                        <div className="space-y-0.5">
                            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Descripción del Tipo</span>
                            <span className="text-sm font-medium text-slate-600 block select-text">
                                {typedevice.description || 'Sin descripción asignada'}
                            </span>
                        </div>
                    </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-2">
                    <span className="inline-flex items-center bg-slate-100 text-slate-500 font-mono text-[10px] px-2 py-0.5 rounded-md font-bold tracking-wider">
                        ID: {typedevice.id} - Creado: {formatFecha(typedevice.createdDate)}
                    </span>
