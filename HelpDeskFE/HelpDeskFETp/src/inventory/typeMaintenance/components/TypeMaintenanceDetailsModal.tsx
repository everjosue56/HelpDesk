...
import type { TypeMaintenanceItem } from '../hooks/useTypeMaintenance';

interface TypeMaintenanceDetailModalProps {
    isOpen: boolean;
    onClose: () => void;
    typeMaintenance: TypeMaintenanceItem | null;
}

export const TypeMaintenanceDetailModal: React.FC<TypeMaintenanceDetailModalProps> = ({
    isOpen,
    onClose,
    typeMaintenance,
}) => {
    if (!typeMaintenance) return null;
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="sm:max-w-md w-[90%] rounded-2xl p-6 bg-white border border-gray-100 shadow-2xl gap-0 select-none animate-in fade-in-50 zoom-in-95 duration-200">
                ...
                <div className="py-5 space-y-4 text-left">
                    ...
                    {/* Fila: Nombre del Tipo de Mantenimiento */}
                    <div className="flex gap-3 items-start">
                        <Settings2 className="h-4 w-4 mt-0.5 text-gray-400 shrink-0" />
                        <div className="space-y-0.5">
                            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Tipo de Mantenimiento</span>
                            <span className="text-sm font-semibold text-slate-700 block select-text">{typeMaintenance.name}</span>
                        </div>
                    </div>

                    {/* Fila: Tiempo Estimado de la Tarea */}
                    <div className="flex gap-3 items-start">
                        <Clock className="h-4 w-4 mt-0.5 text-gray-400 shrink-0" />
                        <div className="space-y-0.5">
                            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block">Tiempo Estimado</span>
                            <span className="text-sm font-medium text-slate-600 block select-text">
                                {typeMaintenance.estimatedTime || 'Sin tiempo estimado asignado'}
                            </span>
                        </div>
                    </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-2">
                    <span className="inline-flex items-center bg-slate-100 text-slate-500 font-mono text-[10px] px-2 py-0.5 rounded-md font-bold tracking-wider">
                        ID: {typeMaintenance.id} - Creado: {formatFecha(typeMaintenance.createdDate)}
                    </span>
